...
def getTextToAnalyze():
    ...
    if counter == 0:
        convo.insert(tk.END, f'\nComp: Would you like to read text to be analyzed from a text file, or manually input it? '
                                f'\nComp:(Enter "0" for text file or "1" for manual entry).')
        counter += 1
    ...
    if not manual and manual is not None:
        ...
        if counter == 2:
            convo.insert(tk.END, f'\nComp: Is the text you would like to analyze in: "{filename}" in the file "{PATH}"?'
                                    f'\nComp: (Enter "Y" to confirm or "N" to reject)')
            counter += 1
...
def analyzeText():
    ...
    '''
    global userEntries, textToBeAnalyzed, kw_dicts, counter, STAGE, csvInput, PATH, manual, scores
    backUp = []
    if counter == 0:
...
def writeToCanvasCSV(assignmentName):
    ...
    PATH = 'Version 3.1/canvas/'
    tempPATH = PATH + 'temp/'
    newCSVFull = PATH + '' + newFileName

    csvFull = PATH + '' + csvName
    ...
    try:
        ...
        with open(csvFull, 'rt') as file:
            ...
            colLet = 'A'  # default case
            colCount = 65  # 65 is the ascii of "A", the first column in Excel
            for row in reader:
                for field in row:
