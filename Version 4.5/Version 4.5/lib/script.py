...
-df is the dateframe thatt stores entries from the responseSpreadsheet
...
-en_dictionary is a dictonary storing ~45,000 english words to be used to compare uniqueness.
'''
userEntries = []
counter = 0
numCat = 0
...
welcome_message = f'Welcome to Text Analyzer Version {version}!' \
                  ...
                  f'\nThe spreadsheet & keyword file must be in the directory "{PATH}" for the program to work. If the files are not in this directory, please move them now.' \
                  f'\nYou should also have the directory "{canvas_PATH}". This directory should contain both a quiz export from Canvas and a gradebook export from canvas. If the files are not in this directory, please move them now. ' \
...
def preprocess(full_path):
    ...
    # If .xlsx does not exist or incorrect name was entered
        ...
        try:
            ...
            convo.insert(tk.END,
                         f'\nComp: The file "{keywordsFile}" could not be found in the directory "{keyword_path}". Please move the file to the directory or confirm you have entered the correct filename in the spreadsheet.\n')
            reset()
            ...
            try:
                ...
                convo.insert(tk.END,
                             f'\nComp: The file "{quizExport}" could not be found in the directory "{canvas_PATH}". Please move the file to the directory or confirm you have entered the correct filename in the spreadsheet.\n')
                reset()
    ...
    if manual:
        if counter == 0:
            ...
            for i in range(0, len(studentEntries)):
                ...
                uniqueWords, totalWords = getUniqueWords(studentEntries[i])
...
def getUniqueWords(studentEntry):
    ...
    # Convert text to array to iterate through each word
    words = studentEntry.lower().split()
...
middle_frame.place(relx=0.01, rely=0.13, relwidth=0.98, relheight=0.75)
