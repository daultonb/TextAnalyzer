"""
Code written by Daulton Baird,
BSc Computer Science student
at the University of British Columbia - Okanagan Campus
As part of a project for makerspaceUBCO
"""
...
-responseSpreadsheet is the spreadsheet with prepared responses to automated the prompts for the marker
-df is the dataframe that stores entries from the responseSpreadsheet
-readCSV is used to determine whether the user (who manually entered keywords) wants to automatically read textToAnalyze from csv
-uniqueThresh is the percentage threshold for what percentage of unique words the text must contain.
-en_dictionary is a dictionary storing ~45,000 english words to be used to compare uniqueness.
"""
userEntries = []
counter = 0
numCat = 0
...
en_dictionary = {}
LINE_SPACE = '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
welcome_message = f'''Welcome to Text Analyzer Version {version}!
\nInstructions\n{LINE_SPACE}
...
The spreadsheet & keyword file must be in the directory "{PATH}" for the program to work. 
If the files are not in this directory, please move them now. 
You should also have the directory "{canvas_PATH}". 
This directory should contain both a quiz export from Canvas and a grade book export from canvas. 
If the files are not in this directory, please move them now.
For more information on formatting and directory organization, see the README file.
{LINE_SPACE}\n'''
"""
Tkinter Variables
    ...
    if manual:
        ...
        # count=2 means they entered a number of categories, confirm the entry
            ...
            try:
                ...
                if numCat == 1:
                    convo.insert(tk.END,
                                 f'\nComp: You have requested {numCat} category, is this correct? '
                                 f'(Enter "Y" to confirm or "N" to change)')
                else:
                    convo.insert(tk.END,
                                 f'\nComp: You have requested {numCat} categories, is this correct? '
                                 f'(Enter "Y" to confirm or "N" to change)')
        ...
        # unless 'n' was entered at count=1, then we reset the list and start again.
            ...
            """
              ...
              then we save the list up to the
            ...
            try:
                ...
                try:
                    ...
                    convo.insert(tk.END, f'\nComp: Keyword file "{fullname}" not found in "{PATH}". '
                                         f'Please move the file to the directory or confirm you have '
                                         f'entered the correct filename into the spreadsheet.\n')
                    counter -= 1
                    getResponse(STAGE)
...
def preprocess(full_path):
    """
    This function preprocesses the entries in the prepared spreadsheet and checks if the files are in the
      correct directories.
    ...
    -df is used to store the dataframe to be used for other functions to access the responses.
    -en_dictionary is a python dictionary that is assigned entries in this function based on file entered in response spreadsheet.
    -uniqueThresh is the percentage threshold for what percentage of unique words the text must contain. It is set in this method.
    """
    global PATH, canvas_PATH, df, en_dictionary, uniqueThresh
    responses = []
    ...
    # If .xlsx does not exist or incorrect name was entered
        ...
        convo.insert(tk.END,
                     f'\nComp: The file "{responseSpreadsheet}" could not be found in the directory "{PATH}". '
                     f'Please move the file to the directory or confirm you have '
                     f'entered the correct filename .\n')
        reset()
        ...
        try:
            ...
            convo.insert(tk.END,
                         f'\nComp: The file "{keywordsFile}" could not be found in the directory "{keyword_path}". '
                         f'Please move the file to the directory or confirm you have '
                         f'entered the correct filename in the spreadsheet.\n')
            reset()
            ...
            try:
                ...
                convo.insert(tk.END,
                             f'\nComp: The file "{quizExport}" could not be found in the directory "{canvas_PATH}". '
                             f'Please move the file to the directory or confirm you have '
                             f'entered the correct filename in the spreadsheet.\n')
                reset()

            else:
                # Check for unique String in quiz Export
                containsString = False
                uniqueString = df['Response'][2]
                df2 = pd.read_csv(quizExport_path)
                for column in df2:
                ...
                if not containsString:
                    convo.insert(tk.END,
                                 f'\nComp: The unique string "{uniqueString}" could not be found in the file "{quizExport}".'
                                 f'Please confirm you have entered the correct string into the spreadsheet.\n')
                    reset()
                else:
                    ...
                    if responses[4].lower() == 'yes':
                        ...
                        try:
                            ...
                            convo.insert(tk.END,
                                         f'\nComp: The file "{gradesExport}" could not be found in the directory "{canvas_PATH}". Please move the file to the directory or confirm you have entered the correct filename in the spreadsheet.\n')
                            reset()
        # Check for English dictionary, and set the global variable if it exists.
        dictionary = responses[7]
        uniqueThresh = int(responses[8])
        dictPath = PATH + dictionary
        try:
            file = open(dictPath)

            # This creates a dictionary of over 40,000 english words
            for word in file.read().split('\n'):
                word = word.lower()
                en_dictionary[word] = 0
            file.close()
        except FileNotFoundError:
            convo.insert(tk.END,
                         f'\nComp: The file dictionary file "{dictionary}" could not be found in the directory "{dictPath}". Please move the file to the directory or confirm you have entered the correct filename in the spreadsheet.\n')
            reset()
...
def createDicts():
    ...
    # If previous keywords have been confirmed, move on to next
        ...
        convo.insert(tk.END, f'\nComp: Enter the keywords of the category named "{kw_dicts[counter].getName()}"'
                             f'\nComp: (In the form "keyword1,keyword2" '
                             f'or "key word1,key word2" for multi-word keywords):')
        counter += 1
...
def getTextToAnalyze():
    ...
    if manual:
        ...
        if readCSV:
            ...
            if counter == 2:
                convo.insert(tk.END,
                             f'\nComp: Is the text you would like to analyze in: "{filename}" in '
                             f'the directory "{canvas_PATH}"?\nComp: (Enter "Y" to confirm or "N" to reject)')
                counter += 1
        ...
        if counter == 0:
            ...
            if counter == 1:
                ...
                # File is not in proper directory. We should never hit this because of preprocess(), but to be safe.
                    ...
                    convo.insert(tk.END, f'\nComp: File "{filename}" not found in "{canvas_PATH}". '
                                         f'Please move the file to the directory or confirm you have entered the '
                                         f'correct filename into the spreadsheet.\n')
                    for i in range(0, len(userEntries) - 1):
                        backUp.append(userEntries[i])
                    userEntries.clear()
                    userEntries = backUp.copy()
...
def analyzeText():
    ...
    It uses the global variables: userEntries, textToBeAnalyzed kw_dicts, counter, STAGE, catNames,
     catThresh, csvInput, PATH, manual, scores, assignmentName, and df.
        ...
        -uniqueThresh is the percentage threshold for unique words in a text entry (to prevent spamming keywords)
    """
    global userEntries, textToBeAnalyzed, kw_dicts, counter, STAGE, csvInput, PATH, manual, scores, assignmentName, df, uniqueThresh
    backUp = []
    ...
    if manual:
        if counter == 0:
            ...
            for i in range(0, len(studentEntries)):
                ...
                uniqueWords, totalWords, mostUsed = getUniqueWords(studentEntries[i])
                uniquePerc = round((uniqueWords / totalWords) * 100, 2)
                if uniquePerc < uniqueThresh:
                    cautionCases.append(i + 1)
                ...
                for j in range(0, len(kw_dictList)):
                    allMatches += categoryCounts[i]
                    allPassFails.append(kw_dicts[i].getPassFail())
                    convo.insert(tk.END, f'\nCategory: "{kw_dicts[j].getName()}",'
                                         f'\nThreshold: {kw_dicts[j].getThresh()},'
                                         f' Total keywords found: {categoryCounts[j]}'
                                         f'\nOccurrences of each Keyword: {kw_dictList[j]}\n'
                                         f'\nPassed Threshold: {kw_dicts[j].getPassFail()}\n')

                categoryCounts.append(allMatches)

                for k in range(0, len(allPassFails)):
                    passedAll = passedAll and allPassFails[k]
                ...
                convo.insert(tk.END, f'\nTotal Keyword Matches: {allMatches}'
                                     ...
                                     f'\nPercentage of Unique words: {uniquePerc}%\n'
                                     f'\nMost used words: {mostUsed[i]}%\n'
                                     f'\n{LINE_SPACE}')

            convo.insert(tk.END, f'Students with less than {uniqueThresh}% unique words: {cautionCases}')
            ...
            convo.insert(tk.END,
                         f'\nComp: Would you like to save student scores (pass/fail) to a Canvas gradebook export? '
                         f'(Enter "Yes" or "No").')
            counter += 1
            getResponse(STAGE)
...
def writeToCanvasCSV(assignmentName):
    ...
    This is done by traversing the export csv file to fin which column the assignment name is in,
    ...
    try:
        ...
        convo.insert(tk.END, f'\nComp: The canvas export file "{csvName}" cold not be found in "{csvFull}". '
                             f'Please export the file from Canvas and move the file to the directory or confirm you '
                             f'have entered the correct filename into the spreadsheet.\n')

    else:
        colLet = 'G'  # first assignment column in Canvas Quiz csv
...
def reset():
    """
    This function resets all modified global variables. It is used at the end of a run when the computer prompts
      the user to enter "C" to continue.
    """
...
def getUniqueWords(studentEntry):
    """
    This function creates a dictionary of over 40,0000 English words (from https://inventwithpython.com/dictionary.txt)
      to check how many unique words the give text to analyze contains.
    The global variables used are:
    -PATH: the directory of the dictionary file
    -en_dictionary: a Python dictionary with ~40,000 English words in it.
    """
    global PATH, en_dictionary
    mostUsed = {}
    unique = 0

    # this line splits the input text into individual words and puts that into an array
    # we also use the lowercase version to prevent capitalized letters from being skipped
    words = re.split('\W+', studentEntry.lower())
    #words = studentEntry.split(" ")
    # Iterate through each word and check if in dictionary
    for word in words:
        if word in en_dictionary:
            en_dictionary[word] += 1

    # Check how many unique words were found from dictionary
    for word in en_dictionary:
        if en_dictionary[word] >= 1:
            unique += 1
            mostUsed[word] = en_dictionary[word]

    mostUsed = sorted(mostUsed.items(), key=lambda x: x[1], reverse=True)

    print(f"MostUsed: {mostUsed}")

    return unique, len(words), mostUsed


def clear():
    """
    This function clears the text in the conversation text field. This is used when the reset button is pushed.
    """
    convo.config(state='normal')
    convo.delete('14.0', tk.END)
    reset()
    getResponse(STAGE)
...
def setColour(colourmode):
    ...
    txt is all text in the program
    """
    global bgcolour, textcolour, scrlcolour, framecolour, titlecolour
...
window.title(f'TextAnalyzer v{version}')

canvas = tk.Canvas(window, height=HEIGHT, width=WIDTH, bg=bgcolour)
canvas.pack(fill='both', expand=True)

header_frame = tk.Frame(window, bg=bgcolour, bd=6)
header_frame.place(anchor='n', relx=0.5, rely=-0.001, relwidth=1, relheight=0.25)

title = tk.Label(header_frame, text='Text Analyzer', font=('Times', 40), bg=bgcolour, fg=titlecolour)
title.place(anchor='n', relx=0.5, rely=0, relwidth=1, relheight=0.35)

credit = tk.Label(header_frame, text='Created by Daulton Baird', font=('Times', 12), bg=bgcolour, fg=titlecolour)
...
lmButton.place(relx=0, rely=0.02, relwidth=0.07, relheight=0.2)

dmButton = ttk.Button(header_frame, text='DarkMode', command=lambda: setColour('dark'))
dmButton.place(relx=0.08, rely=0.02, relwidth=0.07, relheight=0.2)

ubcButton = ttk.Button(header_frame, text='UBCMode', command=lambda: setColour('ubc'))
ubcButton.place(relx=0.16, rely=0.02, relwidth=0.07, relheight=0.2)

clrButton = ttk.Button(header_frame, text='Reset', command=lambda: clear())
clrButton.place(relx=0.929, rely=0.02, relwidth=0.07, relheight=0.2)

middle_frame = tk.Frame(window, bg=framecolour, bd=6)
middle_frame.place(relx=0, rely=0.13, relwidth=1, relheight=0.76)
...
convo = tk.Text(middle_frame, width=10, height=10, wrap="word", yscrollcommand=scrollbar.set,
                borderwidth=0, bg=scrlcolour, fg=textcolour, font=16, )
...
lower_frame.place(anchor='sw', relx=0, rely=1, relwidth=1, relheight=0.11)

user = tk.Entry(lower_frame, bg=scrlcolour, font=40, fg=textcolour)
